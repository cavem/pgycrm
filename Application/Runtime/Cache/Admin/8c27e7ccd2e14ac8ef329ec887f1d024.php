...
$(function(){
    $('.add').click(function(){
        var confirm=function(index,layero){
            ...
            layer.load();
            $.post('/Room/new',formdata,function(data,status){
                if(data==0){
        ...
        myalert('新增机房','900px','660px',"<?php echo U('Room/new');?>",confirm);
    })
    $('.view').click(function(){
        ...
        myalert('查看角色','900px','660px',"<?php echo U('Room/view');?>"+"?id="+id,confirm);
    })
    $('.edit').click(function(){
        var name=$(this).closest('tr').find('.name').text();
        ...
        var confirm=function(index,layero){
            ...
            var formdata = body.find('.mform').serialize();
            layer.load();
            $.post('/Room/edit',formdata,function(data,status){
                if(data==0){
        ...
        myalert('修改机房【'+name+'】','900px','660px',"<?php echo U('Room/edit');?>"+"?id="+id,confirm);
    })
    $('.remove').click(function(){
        ...
        layer.confirm('确定删除【'+name+'】?', {
            ...
            layer.load();
            $.post('/Room/remove',{"id":id},function(data,status){
                if(data==0){
    ...
    $('.gopage').click(function(){
        var cpage=$('.cpage').val();
        window.location.href="<?php echo U('Room/list');?>"+"?p="+cpage;
    })
})
</script>
<div id="content" class="wow fadeIn">
    <div id="content-header">
        <span class="header-title"><i class="fa fa-building-o"></i> 机房管理</span>
        <div class="mysearchbox fr">
            <i class="fa fa-search" style="position: absolute;top: 11px;left: 11px;"></i>
            <form class="sform" action="<?php echo U('Room/list');?>" method="POST">
    ...
    <div class="container-fluid">
        <div class="someopt mt15">
            ...
            <?php if($perm['资源管理']['机房管理']['新增']): ?><button class="btn btn-primary fl add mr10"><i class="fa fa-plus"></i> 新增</button><?php endif; ?>
            <div class="page fr mr10 pages"><?php echo ($page); ?></div>
        </div>
        <table class="table table-hover">
            <thead>
                <tr>
                <th>编号</th><th>机房名称</th><th>运营商</th><th>机房类型</th><th>机房地址</th><th>值班电话</th><th>排序</th><th>操作</th>
                </tr>
            </thead>
            <tbody>
                <?php if(is_array($roomlist)): $k = 0; $__LIST__ = $roomlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                        ...
                        <td style="width: 200px;">
                            <?php if($perm['资源管理']['机房管理']['查看']): ?><button class="btn btn-sm btn-info view"><i class="fa fa-eye"></i></button><?php endif; ?>
                            <?php if($perm['资源管理']['机房管理']['修改']): ?><button class="btn btn-sm btn-warning edit"><i class="fa fa-edit"></i></button><?php endif; ?>
                            <?php if($perm['资源管理']['机房管理']['删除']): ?><button class="btn btn-sm btn-danger remove"><i class="fa fa-trash"></i></button><?php endif; ?>
                        </td>
