...
<div id="content" class="wow fadeIn">
    <div id="content-header">
        <span class="header-title"><i class="fa fa-question-circle-o"></i> 机器型号</span>
        <div class="mysearchbox fr">
            <i class="fa fa-search" style="position: absolute;top: 11px;left: 11px;"></i>
            <form class="sform" action="<?php echo U('Account/list');?>" method="POST">
    ...
    <div class="container-fluid">
        <div class="someopt mt15">
            ...
            <?php if($perm['资源管理']['机器型号']['新增']): ?><button class="btn btn-primary fl add mr10"><i class="fa fa-plus"></i> 新增</button><?php endif; ?>
            <div class="page fr mr10 pages"><?php echo ($page); ?></div>
        </div>
        <table class="table table-hover">
            <thead>
                <tr>
                <th>编号</th><th>型号</th><th>操作</th>
                </tr>
            </thead>
            <tbody>
                <?php if(is_array($modelslist)): $k = 0; $__LIST__ = $modelslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                        <td><?php echo ($vo["id"]); ?></td>
                        <td><?php echo ($vo["name"]); ?></td>
                        <td style="width: 200px;">
                            <?php if($perm['资源管理']['机器型号']['查看']): ?><button class="btn btn-sm btn-info view"><i class="fa fa-eye"></i></button><?php endif; ?>
                            <?php if($perm['资源管理']['机器型号']['修改']): ?><button class="btn btn-sm btn-warning edit"><i class="fa fa-edit"></i></button><?php endif; ?>
                            <?php if($perm['资源管理']['机器型号']['删除']): ?><button class="btn btn-sm btn-danger remove"><i class="fa fa-trash"></i></button><?php endif; ?>
                        </td>
